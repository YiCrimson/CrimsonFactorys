        ...
        for (CommandElement element : elements) {
            ...
            try {
                ...
                if (state.equals(args.getState())) {
                    ...
                    args.setState(state);
                    completions.addAll(element.complete(src, args, ctx));
                    if (!element.getClass().equals(OPTIONAL) && !(element instanceof co.crimsonf.yichu.sts.sponge.wondertrade.teslalibs.argument.CommandElement && ((co.crimsonf.yichu.sts.sponge.wondertrade.teslalibs.argument.CommandElement) element).getParser().isOptional())) {
                        break;
                    }
                    args.setState(state);
                }
